...
def guardar_parametros(tfEntrada, tfInicioIntervalo, tfFinalIntervalo, tfErro, tfPrecisao):
    ...
    if funcao(a)*funcao(b)>=0:
        lbErro = Label(janela, text = "Error on regula falsi method")
        ...
        while True:
            ...
            if abs((m_n-aux_erro)/m_n)<=erro:
                ...
                if funcao(a_n)*funcao(m_n)<0:
                    ...
                    lbFalha = Label(janela, text = "The regula falsi method failed")
                    lbFalha.grid(row = 8+i, column = 0)
        lbResultadoFinal = Button(janela, text = "Final result", width = 12)
        lbRF = Label(janela, text = "Root = "+p.format(m_n))
        lbIteracoes = Label(janela, text = "Iterations number = "+str(iteracoes-1))
        lbErro = Label(janela, text = "Error = "+p.format(aux))
...
lbEntrada = Label(janela, text = "Function")
tfEntrada = Entry(janela)
lbInicioIntervalo = Label(janela, text = "Interval start")
tfInicioIntervalo = Entry(janela)
lbFinalIntervalo = Label(janela, text = "Interval end")
